...
class ToolRegistry:
    ...
    def list_tools_by_source(self) -> dict[str, list[str]]:
        """Group tool names by source (native vs MCP plugin)."""
        groups: dict[str, list[str]] = {"native": [], "mcp": {}}
        for name in self._tools:
            if name.startswith("mcp:"):
                _, mcp_name, _ = name.split(":", 2)
                groups["mcp"].setdefault(mcp_name, []).append(name)
            else:
    ...
    def get_execution_log(self, limit: int = 50) -> list[dict]:
        ...
        return [e.to_dict() for e in self._execution_log[-limit:]]
